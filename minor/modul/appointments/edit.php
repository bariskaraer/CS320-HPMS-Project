...
$doctorID		=$_POST["doctorID"];
	...
	$query = $db->prepare("UPDATE appointments SET doctorNo=?,patientNo=?,dateAppointment=?,appointmentDetail=?");
...
$appointmentIDDDD=$r3;
$query = $db->prepare("Select * from appointments where appointmentsID=?");
$query->execute(array($appointmentIDDDD));
if($row = $query->fetch())
{

    $appointmentDetails 		= $row["appointmentDetail"];
    $appointmentDate 		= $row["dateAppointment"];
    $patientID  	= $row["patientNo"];
    $doctorID		=$row["doctorNo"];
    
}


$queryDoctor = $db->prepare("SELECT * from doctors where doctorsID = ?");
$queryDoctor->execute(array($doctorID));
...
$queryPatient->execute(array($patientID));
											...
											<form class="kt-form" method="post"  enctype="multipart/form-data">
												...
												<div class="form-group ">
													<label for="projectinput7">Appointment Details</label>
													    ...
													    <input class="form-control" name="appointmentDetails" placeholder="" value="<? echo $appointmentDetails ?>">
												...
												<div class="form-group ">
													<label for="projectinput7">Date of the Appointment</label>
													    ...
													    <input class="form-control" type="date" name="appointmentDate" value="<? echo $appointmentDate ?>">
													...
													<div class="form-group " id="firstdivision">
					                                   ...
					                                   <select class="form-control required" name="patientID">
															<?php $listS = $db->prepare("SELECT * FROM patients"); 
																 $listS->execute(array());
															?>
															<option value="<? echo $patientID ?>"><? echo $patientName ?> <? echo $patientSurname ?></option> 
													...
													<label for="projectinput7">Doctor</label>
					                                   ...
					                                   <select class="form-control required" name="doctorID">
															<?php $listS = $db->prepare("SELECT * FROM doctors"); 
																 $listS->execute(array());
															?>
															<option value="<? echo $doctorID ?>"><? echo $doctorName ?> <? echo $doctorSurname ?></option>  
															<?php foreach ($listS as $list) { ?>
